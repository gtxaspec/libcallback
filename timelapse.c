...
char *Timelapse(int fd, char *tokenPtr) {
  ...
  if(!p) {
    if(TimeLapseInfo.count >= TimeLapseInfo.numOfTimes) return "[command] [timelapse.c] not operating.";
    snprintf(ResBuf, 255, "[command] [timelapse.c] file: %s\ninterval: %dsec, count: %d/%d\n", TimeLapseInfo.mp4file, TimeLapseInfo.interval, TimeLapseInfo.count, TimeLapseInfo.numOfTimes);
    return ResBuf;
  }
  ...
  if(State != State_Ready) return "[command] [timelapse.c] error :　Already in operation.";
...
static void *TimelapseThread() {
  ...
  while(1) {
    ...
    if(!frameCtrl.buf) {
      CommandResponse(TimelapseFd, "[command] [timelapse.c] error : not enough memory to video buffer");
      goto error;
    }
    ...
    while((Directive < Directive_ToMP4)) {
      ...
      if(ret) fprintf(stderr, "[command] [timelapse.c] error video_get_frame %d\n", ret);
      if(frameCtrl.stat) fprintf(stderr, "[command] [timelapse.c] error video_get_frame frame.sstat %d\n", frameCtrl.stat);
      if(!startTime.tv_sec) {
    ...
    if(IMP_ISP_Tuning_GetSensorFPS(&fps_num, &fps_den)) {
      CommandResponse(TimelapseFd, "[command] [timelapse.c] error : can't get frame rate");
      goto error;
    }
    ...
    for(int i = 0; i < 20; i++) {
      ...
      fprintf(stderr, "[command] [timelapse.c] retry : can't get mp4 handler\n");
      usleep(1000 * 1000);
    }
    if(mp4writeHandler < 0) {
      CommandResponse(TimelapseFd, "[command] [timelapse.c] error : can't get mp4 handler");
      goto error;
    }

    if(mp4write_start_handler(mp4writeHandler, TimeLapseInfo.mp4file, &videoConfig)) {
      CommandResponse(TimelapseFd, "[command] [timelapse.c] error : can't start mp4 handler");
      goto error;
    }
    ...
    if(!mp4write_stop_handler(mp4writeHandler)) {
      ...
      fprintf(stderr, "[command] [timelapse.c] finish %s %d/%d\n", TimeLapseInfo.mp4file, count, TimeLapseInfo.numOfTimes);
    }
    free(frameCtrl.buf);

    struct timeval finish;
    gettimeofday(&finish, NULL);
    fprintf(stderr, "[command] [timelapse.c] start : %d.%06d  finish : %d.%06d\n", start.tv_sec, start.tv_usec, finish.tv_sec, finish.tv_usec);
finalize:
    CommandResponse(TimelapseFd, "ok");
error:
    TimelapseFd = -1;
...
//  if(getppid() != 1) return;
  ...
  if(pthread_create(&thread, NULL, TimelapseThread, NULL)) {
    fprintf(stderr, "[command] [timelapse.c] pthread_create error\n");
    pthread_mutex_unlock(&TimelapseMutex);
    return;
  }
}
